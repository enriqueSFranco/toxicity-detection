...
const { TWITCH_OAUTH_TOKEN, TWITCH_CLIENT_ID, TWITCH_CLIENT_SECRET, ENDPOINT_SEARCH_CHANNEL } = API
...
export const searchChannel = async ({ channelName }: { channelName: string }) => {
  ...
  const channelResponse = await fetch(`https://api.twitch.tv/helix/search/channels?query=${channelName}`, options)
  ...
  return {
    data: mappedChannelJson,
    pagination: channelJson.pagination
  }
}
