...
function Message ({ username, message, color, prediction }: Props) {
  return (
    <div className='max-w-fit flex items-center justify-between gap-4'>
      ...
      <span className={`${prediction ? 'bg-red-300/20 text-red-400' : 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300'} text-xs font-medium px-2.5 py-0.5 rounded-sm inline-block capitalize`}>{prediction ? '👎 tóxico' : ' 👍 bueno'}</span>
    </div>
  )
}
